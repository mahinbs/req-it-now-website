...
export const RequirementsFilter = ({
  ...
  const inputClass = isDark
    ? "pl-10 glass bg-white/5 border-white/20 text-white placeholder:text-slate-400"
    : "pl-10 border-slate-300 text-slate-700 placeholder:text-slate-400 bg-white";
  const buttonClass = isDark
    ? "glass border-white/20 text-slate-200 hover:bg-white/10 hover:text-white hover:border-white/30 transition-all duration-300"
    : "bg-white hover:bg-slate-50 border-slate-300 shadow-sm hover:shadow-md transition-all duration-200";
  const dropdownClass = isDark
    ? "glass bg-slate-800/90 backdrop-blur-xl border-white/20 shadow-2xl rounded-xl p-2"
    : "w-56 bg-white border border-slate-200 shadow-xl rounded-xl p-2";
  const labelClass = isDark
    ? "text-slate-200 font-medium px-2 py-1"
    : "text-slate-700 font-medium px-2 py-1";
  ...
  const clearButtonClass = isDark
    ? "cursor-pointer text-red-400 hover:bg-red-500/20 hover:text-red-300 rounded-lg transition-colors duration-150 font-medium"
    : "cursor-pointer text-red-600 hover:bg-red-50 hover:text-red-700 rounded-lg transition-colors duration-150 font-medium";
  const popoverClass = isDark
    ? "glass bg-slate-800/90 backdrop-blur-xl border-white/20 shadow-2xl"
    : "";
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="flex flex-wrap items-center gap-2">
          ...
          <DropdownMenuContent className={dropdownClass} align="end">
            ...
            {["all", "pending", "ongoing", "completed", "rejected"].map(
              (status) => (
                <DropdownMenuItem
                  key={status}
                  onClick={() =>
                    handleStatusFilterChange(status as StatusFilter)
                  }
            ...
            {["all", "low", "medium", "high"].map((priority) => (
              <DropdownMenuItem
                key={priority}
                onClick={() =>
                  handlePriorityFilterChange(priority as PriorityFilter)
                }
        ...
        <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
          ...
          <PopoverContent
            className={`w-auto p-0 ${popoverClass}`}
            align="start"
          >
