...
export const AuthPage = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        {/* Error Display */}
        {error && <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm shadow-sm animate-in slide-in-from-top duration-300">
            <div className="flex items-start space-x-2">
              ...
              <div className="flex-1">
                <p>{error}</p>
                <button onClick={() => setError(null)} className="mt-2 text-red-700 underline hover:no-underline text-sm font-medium">
                  Dismiss
                </button>
              </div>
            </div>
        ...
        {/* Auth Forms */}
        {authMode === 'login' ? <LoginForm onLogin={handleLogin} onSwitchToSignup={() => setAuthMode('signup')} loading={loading} error={error} /> : <SignupForm onSignup={handleSignup} onSwitchToLogin={() => setAuthMode('login')} loading={loading} error={error} />}
