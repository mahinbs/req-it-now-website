...
export const RequirementsList = ({
  ...
  if (requirements.length === 0) {
    return <Card className="border-slate-600/50">
        <CardContent className="text-center py-12">
          ...
          <Button onClick={handleRefresh} variant="outline" className="mt-4 bg-zinc-700 hover:bg-zinc-600">
  ...
  return <div className="space-y-4">
      {notificationsLoading && <div className="text-center text-sm text-slate-300 py-2">
          Loading notification status...
        </div>}
      
      <div className="grid gap-6 auto-rows-fr">
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 auto-rows-fr">
          {requirements.map(requirement => <RequirementCard key={requirement.id} requirement={requirement} onOpenChat={onChatClick} unreadCount={getUnreadCount(requirement.id)} onMarkAsRead={markAsRead} onApprovalUpdate={onApprovalUpdate} onDownloadAttachment={onDownloadAttachment} />)}
        </div>
      </div>
    </div>;
};