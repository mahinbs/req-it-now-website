...
export const NotificationDebugger = () => {
  ...
  const { notificationCounts, connected, refreshNotifications } = useUnifiedNotificationContext();
  ...
  const runDebugCheck = async () => {
    ...
    try {
      ...
      const { data: adminReadStatus, error: adminError } = await supabase
        .from('admin_read_status')
        .select('*')
        .eq('admin_id', user.id);
      ...
      const { data: unreadCounts, error: unreadError } = await supabase.rpc(rpcFunction, rpcParams);
