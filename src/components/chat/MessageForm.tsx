...
export const MessageForm = ({
  ...
  return <div className="space-y-3">
      {selectedFile && <div className="flex items-center justify-between p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200 shadow-sm">
          <div className="flex items-center space-x-3 flex-1 min-w-0">
            <div className="relative">
              <span className="text-xl">{getFileIcon(selectedFile.type)}</span>
              {fileValidated && <CheckCircle className="h-4 w-4 text-green-500 absolute -top-1 -right-1 bg-white rounded-full" />}
            </div>
          ...
          <Button type="button" variant="ghost" size="sm" onClick={removeSelectedFile} className="h-7 w-7 p-0 ml-2 hover:bg-red-100 text-slate-600 hover:text-red-600" disabled={isLoading}>
            <X className="h-3 w-3" />
          </Button>
        </div>}
      ...
      <form onSubmit={handleSubmit} className="flex space-x-2">
        <Input value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder={selectedFile ? "Add a message (optional)" : "Type your message..."} disabled={isLoading || disabled} autoComplete="off" className="flex-1 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-zinc-950" />
        <AttachmentButton onFileSelect={handleFileSelect} disabled={isLoading || disabled} />
        <Button type="submit" size="sm" disabled={!canSend} className={`flex items-center space-x-1 px-4 transition-all duration-200 ${isLoading ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 transform hover:scale-105'}`}>
          {isLoading ? <>
